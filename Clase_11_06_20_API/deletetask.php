...
if($method==='DELETE'){ // se utiliza put por que se va actualizar información a la base de datos
...
$sql="DELETE FROM tasks WHERE Id={$id}"; // instrucción sql para eliminar registros en la base de datos
    $result=$conn->query($sql); // se activa la conexión $conn y se ejecutar el query $sql

    echo json_encode(array('response'=>'Task deleted successfully.'));// se le da respuesta en formato json al usuario. Siempre hay que dar una respuesta ojo. 
